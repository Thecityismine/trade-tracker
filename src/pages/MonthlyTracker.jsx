...
function MonthCalendar({ month }) {
  const year = month.monthStart.getFullYear();
  const monthNum = month.monthStart.getMonth();
  ...
  month.trades.forEach((trade) => {
    ...
    if (!dayMap[dayKey]) dayMap[dayKey] = { pnl: 0, trades: 0 };
    dayMap[dayKey].pnl += trade.gainLoss || 0;
